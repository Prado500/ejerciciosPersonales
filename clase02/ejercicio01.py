...
def registrarEmpleado (nombre: str, cargo: str, salario):
    ...
    if not nombre or not cargo or salario <= 0 or not salario or not isinstance(salario, int or float): # OJO! Si pongo or or not isinstance(salario, int) or not isinstance(salario, float), vuelvo la condicion mutuamente excluyente y nunca va a guardar el dato
